    ...
    for dirPath, dirNames, fileNames in os.walk(sourcePath):
        ...
        for fileName in fileNames:
            ...
            try:
                ...
                if os.path.getmtime(replicaFilePath)!=os.path.getmtime(sourceFilePath):
                    shutil.copy2(sourceFilePath,replicaFilePath)
                    logging.info(f"Updated copy of {sourceFilePath} at {replicaFilePath}")
        ...
        for dirName in dirNames:
            ...
            if not os.path.exists(replicaDirPath):
                #copy new source folder dir and its contents in replica
                createReplica(f"{sourcePath}/{dirName}",replicaDirPath)
            else:
                #updated source folder dir in replica
...
if __name__ == '__main__':
    ...
    logging.basicConfig(level=logging.INFO, filename=f"{logPath}/log.log",filemode="a",format="%(asctime)s %(levelname)s %(message)s")    
